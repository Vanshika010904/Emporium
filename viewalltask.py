...
class viewAddTask():
    ...
    def manageTask(self,data):
        ...
        # print("self.data in view all task ",self.data[0])#using this print to chheck the id of the logged in user
        temp = (self.data[0])
        data = database.allEmpTask(self.data[0])
        # print("Data in view all task================",data) #using thiss print to print all the data retrievd from the database
        for i in data:
            self.tr.insert('', 0, text = i[0], values=(i[1], i[2],i[3], i[4], i[5],'Complete'))
